...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Contract2025 contract = new Contract2025();
        ...
        autoDiscoverDevices(contract, devicesByBus, dioDevices, devicesByPowerSource);
        // Print report
    ...
    private static void generatePDHReport(Contract2025 contract, SwerveInstance[] swerveModules, List<DeviceInfo> dioDevices) {
        ...
        // Collect subsystem motors
        if (contract.isAlgaeCollectionReady()) {
            CANMotorControllerInfo motor = contract.getAlgaeCollectionMotor();
            if (motor.pdhPort() != null) {
                pdhUsage.put(motor.pdhPort(), motor.name());
            }
        }
        
        if (contract.isCoralCollectionMotorReady()) {
            CANMotorControllerInfo motor = contract.getCoralCollectionMotor();
            if (motor.pdhPort() != null) {
                pdhUsage.put(motor.pdhPort(), motor.name());
            }
        }
        
        if (contract.isCoralArmMotorReady()) {
            CANMotorControllerInfo motor = contract.getCoralArmPivotMotor();
            if (motor.pdhPort() != null) {
                pdhUsage.put(motor.pdhPort(), motor.name());
            }
        }
        ...
        if (contract.isElevatorReady()) {
            CANMotorControllerInfo motor = contract.getElevatorMotor();
            if (motor.pdhPort() != null) {
                pdhUsage.put(motor.pdhPort(), motor.name());
            }
        }
            ...
            try {
                ...
                // Check if method returns DeviceInfo
                    ...
                    if (device != null) {
                        ...
                        // Categorize device based on powerFrom
                        if (device.powerFrom != null && device.powerFrom.equals(PowerSource.RIO)) {
