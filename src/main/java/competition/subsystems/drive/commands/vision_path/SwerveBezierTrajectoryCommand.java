...
public class SwerveBezierTrajectoryCommand extends SwerveBezierTrajectoryBase {
    private final CoprocessorCommunicationSubsystem
    ...
    public void initialize() {
        ...
        if (client != null) {
            ...
            if (curves != null && !curves.getCurvesList().isEmpty()) {
                log.info(
                        "Retrieved drive trajectory from XTABLES, proceeding to drive.");
                subscriber = new CachedSubscriber("bezier_path", client, 1, true);
                final XTableValues.TraversalOptions options =
                        curves.hasOptions() ? curves.getOptions() : null;
                setSegmentedBezierCurve(curves, options);
                super.initialize();
            } else {
                log.warn(
                        "There was no drive trajectory found from XTABLES, cancelling.");
                cancel();
            }
        }
    }
    ...
    @Override
    public void execute() {
        if (subscriber != null && curves != null
                && subscriber.getAsBezierCurves(curves).equals(curves)) {
            super.execute();
